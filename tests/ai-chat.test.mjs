...
test('a stored AI custom-effect payload is ignored and has no executable-code UI', async () => {
  const now = 1700000000000;
  const mock = createChromeMock({
    initialStorage: {
      [AI_CONFIG_KEY]: { provider: 'anthropic', apiKey: 'sk-test', model: 'claude-opus-5' },
  ...
  const { document, errors } = await loadExtensionPage('options/options.html', { chrome: mock.chrome });
  assert.deepEqual(errors, []);

  await waitFor(() => document.querySelector('.ai-answer'), { timeout: 3000 });
  assert.equal(document.querySelector('.ai-chat-effect-preview'), null);
  assert.doesNotMatch(document.body.textContent, /Save as custom effect|保存为自定义动效/);
});

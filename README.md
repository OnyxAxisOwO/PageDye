...
- **深度兼容模式**（v0.6.3）：应对像 Google 移动端这类被多层不透明容器叠满整个视口的顽固网站，自动检测并清除这些遮挡层，无需手动逐个排查
...
有些网站（典型例子是 Google 移动端页面）用好几层不透明容器叠在一起铺满整个视口，背景选择器只能指定一个元素，遮不住上面还有的其他不透明层，无论用整页覆盖还是背景选择器都看不到效果。

打开**高级设置 → 深度兼容模式**并勾选开关即可：PageDye 会在视口上采样探测，自动找出铺满大部分视口且带不透明背景色的容器，把它们的背景清除，并在滚动、窗口变化和页面内容更新时持续重新扫描。

这个模式比较"暴力"，个别情况下可能误清除某个依赖背景色做对比度的元素（比如按钮、弹窗）。遇到这种情况，可以在下方的**排除选择器**里填入该元素的选择器（如 `.modal, [role=dialog]`），把它排除在扫描范围之外。
